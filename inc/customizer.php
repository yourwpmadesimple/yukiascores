 ...
 */
function yukiascores_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'theme_bg_color', 
				array(
					'label'		=> __( 'Header and footer background color', 'yukiascores'),
					'section'	=> 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'interactive_color', array(
				'label'		=> __( 'Interactive color (links etc)', 'yukiascores' ),
				'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options', 
		array(
			'title'			=> __( 'Theme Options', 'yukiascores' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'yukiascores' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'yukiascores_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);

	// Add the controls
	$wp_customize->add_control(	'yukiascores_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Index/archive displays', 'yukiascores' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Excerpt (default)', 'yukiascores' ),
				'full-content'	=> __( 'Full content', 'yukiascores' )
			),
...
add_action( 'customize_register', 'yukiascores_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function yukiascores_customize_preview_js() {
	wp_enqueue_script( 'yukiascores_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'yukiascores_customize_preview_js' );
...
function yukiascores_sanitize_length( $value ) {
...
if ( ! function_exists( 'yukiascores_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see yukiascores_custom_header_setup().
 */
function yukiascores_header_style() {
